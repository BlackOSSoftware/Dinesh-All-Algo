...
def test_invert_grid_labels():
    from app.services.grid_logic import build_grid_levels

    normal = build_grid_levels(
        reference_price=307,
        grid_gap=2,
        levels_above=3,
        levels_below=3,
    ...
    u1n = next(l for l in normal if l.level_id == "U1")
    d1n = next(l for l in normal if l.level_id == "D1")
    u1i = next(l for l in inverted if l.level_id == "U1")
    d1i = next(l for l in inverted if l.level_id == "D1")
    assert u1n.action_label == "Up Exit / Down Buy 2"
    assert d1n.action_label == "Down Add / Base Exit 2"
    assert u1i.action_label == "Up Buy 2 / Down Exit"
    assert d1i.action_label == "Up Exit / Down Add 2"


def test_invert_grid_u_ladder_opposite_of_normal():
    cfg = {**_cfg(300), "invertGrid": True}
    rt = default_runtime()
    rt, boot = bootstrap_initial_entry({**cfg, "grid_runtime": rt}, rt, fill_price=300)
    actions = list(boot)
    for px in [300.0, 302.0, 304.0, 306.0, 304.0, 302.0, 300.0]:
        rt, acts = process_price_tick({**cfg, "grid_runtime": rt}, rt, px)
        actions.extend(acts)
    pairs = [(a["action"], a["level"]) for a in actions]
    assert pairs == [
