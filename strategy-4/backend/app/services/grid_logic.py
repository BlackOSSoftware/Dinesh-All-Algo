"""Build grid levels and process price ticks for Strategy 4 MCX grid."""
    ...
    for i in range(levels_above, 0, -1):
        levels.append(
            GridLevel(
                ...
                action_label=f"Up Exit / Down Buy {lots_per_grid}",
                kind="upper",
            )
        )
    levels.append(
    ...
    for i in range(1, levels_below + 1):
        levels.append(
            GridLevel(
                level_id=f"D{i}",
                price=round(reference_price - i * grid_gap, 4),
                action_label=f"Down Add / Base Exit {lots_per_grid}",
                kind="lower",
            )
        )
...
def _ui_status(level: GridLevel, phase: LevelPhase, *, base_entered: bool, position: int, arm_locked: bool) -> str:
    if level.kind == "base":
        if not base_entered:
            return "Pending"
...
def compute_next_action_level(levels: list[GridLevel], runtime: dict[str, Any], current_price: float) -> str | None:
...
def bootstrap_initial_entry(
    ...
    *,
    ...
    rt["nextActionLevel"] = compute_next_action_level(levels, rt, entry_px)
...
def _process_single_price_step(
    ...
    for level, direction in crosses:
        if level.kind == "upper" and direction == "up":
            idx = _upper_idx(level.level_id)
            deeper = f"U{idx + 1}"
            if not upper_arm_locks.get(level.level_id) or deeper in crossed_up_ids:
        ...
        action_type, delta, msg, new_phase, neutralize_level = _action_for_cross(
            level,
            direction,
    ...
    if not any(_level_phase(level_states, f"U{i}") == "sold" for i in range(1, max_upper + 1)):
        upper_peak_sold = 0
    ...
    rt.update(
        {
            ...
            "nextActionLevel": compute_next_action_level(levels, rt, current_price),
        }
    )
    return rt, actions
