...
def get_dashboard(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    grid_levels = [
        GridLevelOut(level=r["level"], price=r["price"], action=r["action"], status=r.get("status") or "")
        for r in strategy_rows
    ]

    position_lots = int(runtime.get("positionLots") or 0)
    avg_entry = float(runtime.get("entryPrice") or 0)
    side = runtime.get("side")
    if position_lots > 0 and current_price > 0 and avg_entry > 0:
        unrealized = (current_price - avg_entry) * position_lots if side == "BUY" else (avg_entry - current_price) * position_lots
    else:
        unrealized = 0.0
...
def run_backtest(body: BreakoutBacktestRunIn, _user: User = Depends(get_current_user)):
    """Run MCX breakout backtest on Angel historical 1-min candles."""
    try:
        result = run_breakout_backtest(body.model_dump())
