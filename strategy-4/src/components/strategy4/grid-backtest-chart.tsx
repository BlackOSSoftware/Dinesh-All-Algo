...
import type { GridBacktestCandle, GridBacktestTrade, GridLevelRow } from "@/lib/strategy4/types";
...
function buildMarkers(trades: GridBacktestTrade[], candles: GridBacktestCandle[]): Marker[] {
  ...
  return trades.map((t) => ({
    ...
    detail: `#${t.id} ${t.action.replaceAll("_", " ")} · Fill ${fmtPx(t.fillPrice ?? 0)} · ${t.lots}L`,
    tradeId: t.id,
    action: t.action,
    level: t.action,
  }));
...
function levelColor(kind: string | undefined, level: string) {
  switch (kind) {
    case "reference":
      return "var(--accent)";
    case "buy_trigger":
    ...
    case "exit":
      return "#f97316";
    default:
      return level === "REF" ? "var(--accent)" : "var(--text-muted)";
  }
}

function actionColor(action: string) {
  if (action.includes("INITIAL_BUY") || action.includes("REVERSE_BUY")) return "var(--success)";
  if (action.includes("INITIAL_SELL") || action.includes("REVERSE_SELL")) return "var(--danger)";
  if (action.startsWith("EXIT_TP")) return "#22c55e";
  if (action.startsWith("EXIT_SL")) return "#ef4444";
  return "var(--accent)";
}
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap gap-4 text-[10px] text-[var(--text-muted)]">
          ...
          <span className="inline-flex items-center gap-1.5 text-[var(--accent)]">— Reference / triggers</span>
      ...
      <div className="rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-muted)]">
        ...
        >
          ...
          {gridLevels.map((g, gi) => {
            const kind = (g as { kind?: string }).kind;
            const color = levelColor(kind, g.level);
            const isRef = g.level === "REF" || kind === "reference";
            const label = (g as { label?: string }).label || g.level;
            const levelKey = (g as { id?: number }).id ?? `${g.level}-${g.price}-${gi}`;
            return (
              <g key={levelKey}>
                <line
                  x1={padL}
                  ...
                  y2={yAt(g.price)}
                  stroke={color}
                  strokeWidth={isRef ? 1.6 : 1}
                  strokeDasharray={isRef ? "0" : "7 5"}
                  opacity={isRef ? 1 : 0.85}
                />
                <text x={padL + plotW + 8} y={yAt(g.price) + 3} fontSize="9" fill={color} fontWeight={isRef ? "700" : "500"}>
                  {label} {fmtPx(g.price)}
                </text>
              </g>
            );
