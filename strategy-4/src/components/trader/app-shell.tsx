...
import { fetchSettings, setAlgoRunning } from "@/lib/strategy4/api";
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const [algoCheckPending, setAlgoCheckPending] = useState(true);
        ...
        <div className="flex h-svh min-h-0 overflow-hidden bg-[var(--surface-base)]">
          <aside className="flex h-full w-[220px] shrink-0 flex-col border-r border-[var(--border-subtle)] bg-[var(--surface-elevated)]">
            <div className="flex items-center justify-between border-b border-[var(--border-subtle)] px-4 py-4">
              <p className="text-sm font-semibold text-[var(--text-primary)]">Strategy 4</p>
              <button
                type="button"
                onClick={toggleTheme}
              ...
              <div className="flex items-center justify-between gap-3 px-1">
                <div>
                  ...
                  <p className="text-[11px] text-[var(--text-muted)]">
                    {algoCheckPending ? "Syncing…" : algoRunning ? "Running" : "Stopped"}
                  </p>
                </div>
