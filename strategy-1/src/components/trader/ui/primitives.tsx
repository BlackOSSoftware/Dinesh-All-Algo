...
export function MetricBadge({
  ...
  return (
    ...
    >
      ...
      <span className={cn("mt-1 font-semibold tracking-tight", size === "lg" ? "text-xl sm:text-2xl" : "text-base")}>
        {value}
      </span>
    </div>
  );
...
export function SummaryStatCard({
  ...
  return (
    <PremiumCard hover className="relative overflow-hidden">
      ...
      <p className="relative mt-4 text-[11px] font-bold uppercase tracking-[0.16em] text-[var(--text-muted)]">{label}</p>
      <div className="relative mt-2 text-2xl font-semibold tracking-tight text-[var(--text-primary)] sm:text-3xl">{value}</div>
...
export function StatusPillLarge({
  ...
  return (
    <span className={cn("inline-flex items-center rounded-full px-4 py-2 text-sm font-bold tracking-wide", active ? activeClass : inactiveClass)}>
      {label}
    </span>
  );
}
  ...
  return (
    <span className="group relative ml-1 inline-flex align-middle">
      <button
        ...
        className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-[var(--border-subtle)] text-[10px] font-bold text-[var(--text-muted)] hover:border-[var(--accent)] hover:text-[var(--accent)]"
      >
        i
      </button>
      <span
        role="tooltip"
        className="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 hidden w-56 -translate-x-1/2 rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-elevated)] px-3 py-2 text-[11px] font-normal leading-snug text-[var(--text-secondary)] shadow-lg group-hover:block group-focus-within:block"
      >
        {text}
      </span>
    </span>
...
export function FieldLabel({ label, help }: { label: string; help?: string }) {
  return (
    <span className="inline-flex items-center text-[10px] font-bold uppercase tracking-wider text-[var(--accent)]">
      {label}
      {help ? <InfoTip text={help} /> : null}
    </span>
  );
