  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 pb-10">
      ...
      <PremiumCard className="!p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <FloatingField id="set-entry-trigger" label="Entry Trigger Points" help={SETTINGS_HELP.entryGap} type="number" value={displayField(d.entryGap, d.bootDone)} placeholder="191" onChange={(v) => { const n = parseInt(v, 10); if (!v || !Number.isFinite(n) || n < 1) return; d.setEntryGap(n); }} />
          <FloatingField id="set-strike-offset" label="Strike Offset" help={SETTINGS_HELP.strikeOffset} type="number" value={displayField(d.strikeOffset, d.bootDone)} placeholder="200" onChange={(v) => { const n = parseInt(v, 10); if (!v || !Number.isFinite(n) || n < 50) return; d.setStrikeOffset(n); }} />
          <FloatingField id="set-stop-dist" label="Stop Distance (Index)" help={SETTINGS_HELP.stopDistance} type="number" value={displayField(d.stopDistance, d.bootDone)} placeholder="191" onChange={(v) => { const n = parseInt(v, 10); if (!v || !Number.isFinite(n) || n < 1) return; d.setStopDistance(n); }} />
        </div>
      </PremiumCard>
      ...
      <PremiumCard className="!p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <FloatingField id="set-max-entries" label="Maximum Entries" help={SETTINGS_HELP.numEntries} type="number" value={displayField(d.numEntries, d.bootDone)} placeholder="4" onChange={(v) => { const n = parseInt(v, 10); if (!v || !Number.isFinite(n) || n < 1) return; d.setNumEntries(n); d.setEntryLots(normalizeEntryLots(d.entryLots, n, d.entryLots[0] ?? d.initialLots, d.entryLots[1] ?? d.addLots)); }} />
          ...
          <FloatingField id="set-avg-gap" label="Averaging Gap" help={SETTINGS_HELP.addGap} type="number" value={displayField(d.addGap, d.bootDone)} placeholder="45" onChange={(v) => { const n = parseInt(v, 10); if (!v || !Number.isFinite(n) || n < 1) return; d.setAddGap(n); }} />
        </div>
      </PremiumCard>
      ...
      <PremiumCard className="!p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <FloatingField id="set-first-tp1" label="First Entry TP1" help={SETTINGS_HELP.firstEntryTp1} type="number" value={displayField(d.firstEntryTp1Pts, d.bootDone)} placeholder="70" onChange={(v) => { const n = parseInt(v, 10); if (!v || !Number.isFinite(n) || n < 1) return; d.setFirstEntryTp1Pts(n); }} />
          <FloatingField id="set-tp1-pts" label="Averaging TP1" help={SETTINGS_HELP.target1Pts} type="number" value={displayField(d.target1Pts, d.bootDone)} placeholder="45" onChange={(v) => { const n = parseInt(v, 10); if (!v || !Number.isFinite(n) || n < 1) return; d.setTarget1Pts(n); }} />
          <FloatingField id="set-tp2-trail" label="TP2 Trail Distance" help={SETTINGS_HELP.tp2Trail} type="number" value={displayField(d.tp2TrailPoints, d.bootDone)} placeholder="30" onChange={(v) => { const n = parseInt(v, 10); if (!v || !Number.isFinite(n) || n < 1) return; d.setTp2TrailPoints(n); }} />
        </div>
      </PremiumCard>
      ...
      <PremiumCard className="!p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <FloatingField id="set-reentry-gap" label="Re-entry Gap" help={SETTINGS_HELP.reEntryGap} type="number" value={displayField(d.reEntryGap, d.bootDone)} placeholder="45" onChange={(v) => { const n = parseInt(v, 10); if (!v || !Number.isFinite(n) || n < 1) return; d.setReEntryGap(n); }} />
      ...
      <PremiumCard className="!p-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <FloatingField id="set-max-reentry" label="Max Re-entries / Day (0 = unlimited)" help={SETTINGS_HELP.maxReEntries} type="number" value={displayField(d.maxReEntries, d.bootDone)} placeholder="3" onChange={(v) => { const n = parseInt(v, 10); if (!Number.isFinite(n) || n < 0) return; d.setMaxReEntries(n); }} />
        </div>
      </PremiumCard>
    </div>
  );
